...
if [[ -n "${EXPLAIN_HOOK_INSTALLED_ZSH:-}" ]]; then
  return 0
fi
...
if [[ -z "${EXPLAIN_STDERR_REDIRECTED:-}" ]]; then
...
EXPLAIN_HOOK_INSTALLED_ZSH=1
explain_maybe_start_daemon
add-zsh-hook preexec explain_preexec
add-zsh-hook precmd explain_precmd
